...
newton<-function (x, y, eps = 1e-06, i=1) {
    fx <-function(x,y){1}
    fy<-function(x,y){1}
    gx<-function(x,y){2*x}
    gy<-function(x,y){4*y}
    d<-function(x,y){fx(x,y)*gy(x,y)-fy(x,y)*gx(x,y)}
    a<-f(x,y)
    b<-g(x,y)
    ...
    if ((a == 0) && (b == 0)){
      ...
      while ((abs(a) >= eps && abs(b) >= eps)){
       x <- x - (((gy(x,y)*f(x,y))-(fy(x,y)*g(x,y))))/d(x,y)
       y <- y - (((fx(x,y)*g(x,y))-(gx(x,y)*f(x,y))))/d(x,y)
       a<-f(x,y)
       b<-g(x,y)
    }
    print((c(x,y)))
...
ysolution<-results[2]
```
From above function, I get the two solutions, $x = -0.8164966$ and $y = -0.8164966$ respectively. 
